...
0 <= amount <= 10^4
 */
public class _87_CoinChange {
    class Solution {
        public int coinChange(int[] coins, int amount) {
            int[] memo = new int[amount + 1];
            int maxChange;
            for (int i = 1; i <= amount; i++) {
                maxChange = Integer.MAX_VALUE;
                for (int j = 0; j < coins.length; j++) {
                    if (i - coins[j] >= 0 && memo[i - coins[j]] != -1 && memo[i - coins[j]] + 1 < maxChange) {
                        maxChange = memo[i - coins[j]] + 1;
                    }
                }
                memo[i] = maxChange == Integer.MAX_VALUE ? -1 : maxChange;
            }
            return memo[amount];
        }
    }}
