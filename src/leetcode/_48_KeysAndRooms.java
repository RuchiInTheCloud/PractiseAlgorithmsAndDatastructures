 ...
 */
public class _48_KeysAndRooms {
    class Solution {
        public boolean canVisitAllRooms(List<List<Integer>> rooms) {
            boolean[] bitVector = new boolean[rooms.size()];
            bitVector[0] = true;
            Queue<List<Integer>> toVisit = new LinkedList<>();
            toVisit.add(rooms.get(0));

            while (!toVisit.isEmpty()) {
                List<Integer> nextRooms = toVisit.remove();
                for (int room : nextRooms) {
                    if (!bitVector[room]) {
                        bitVector[room] = true;
                        toVisit.add(rooms.get(room));
                    }
                }
            }
            for (boolean flag : bitVector) {
                if (!flag) {
                    return false;
                }
            }
